 ...
 */
 ...
 function sendCode(code) {
    ...
    return new Promise(async resolve => {
        $.get({ url: `http://106.13.238.181/jd/code/add/jxlhb/${code}/dauid3as23`, timeout: 10000 }, (err, resp, data) => {
          try {
            if (err) {
              console.log(`${JSON.stringify(err)}`)
              console.log(`${$.nickName || $.UserName} 助力提交失败.`)
            } else {
              if (data) {
                console.log(`${$.nickName || $.UserName} 助力提交成功.`)
                data = JSON.parse(data);
              }
            }
          } catch (e) {
